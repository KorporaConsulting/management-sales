...
class Evencal extends CI_Controller {
       ...
       function add_event(){
              ...
              $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
              $this->load->view('kalender/add_event', $data);
       }
       ...
       function do_add(){
              ...
              $this->form_validation->set_rules('nama_sales', 'Nama Sales', 'trim|required|xss_clean');
            ...
            if ($this->form_validation->run() == FALSE){
             ...
             echo json_encode(array('status' => true, 'time' => $this->input->post('time'), 'nama_sales' => $this->input->post('nama_sales'),'event' => $this->input->post('event')));
            }
       }
