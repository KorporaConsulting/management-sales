...
class Evencal_model extends CI_Model {
     ...
     function getDateEvent($year, $month){
           ...
           function getEvent($year, $month, $day){
            ...
            $query = $this->db->select('idevent as id, event_time as time, nama_sales as nama_sales, event')->order_by('event_time')->get_where('event_detail', array('event_date' => $year));
            if($query->num_rows() > 0){
     ...
     // insert event
     function addEvent($year, $month, $day, $time, $nama_sales, $event){ 
            ...
            if($check->num_rows() > 0){
             ...
             $this->db->insert('event_detail', array('event_date' => "$year-$month-$day", 'event_time' => $time, 'nama_sales' => $nama_sales,'event' => $event));
            }else{
